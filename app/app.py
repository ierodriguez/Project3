...
def index():
    ...
    cursor.execute('SELECT * FROM mlbteamsT')
...
def record_view(team_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM mlbteamsT WHERE id=%s', team_id)
    result = cursor.fetchall()
...
def form_update_post(team_id):
    ...
    sql_update_query = """UPDATE mlbteamsT t SET t.Team = %s, t.Payroll_millions = %s, t.Wins = %s WHERE t.id = %s """
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO mlbteamsT (Team,Payroll_millions,Wins) VALUES (%s, %s,%s) """
...
def form_delete_post(team_id):
    cursor = mysql.get_db().cursor()
    sql_delete_query = """DELETE FROM mlbteamsT WHERE id = %s """
